...
def load_features(feature_name):
    ...
    for classname in CLASSNAMES:
        file_path = os.path.join(
            FEATURE_DIR, "{}_{}.npy".format(classname, feature_name)
        )
...
def get_scaled_umap_embeddings(features, neighbour, distance):
    ...
    embedding = umap.UMAP(
        n_neighbors=neighbour, min_dist=distance, metric="correlation"
...
def plotUMAPmap(embedding, labels):
    ...
    fig, ax = plt.subplots(
        nrows=len(neighbours), ncols=len(distances), figsize=(30, 30)
    )
